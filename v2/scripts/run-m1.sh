...
# tear everything down. Run from v2/:  bash scripts/run-m1.sh
set -u
cd "$(dirname "$0")/.."   # → v2/
ROOT="$(pwd)"
...
redis-cli ping >/dev/null 2>&1 || { echo "[run-m1] redis not reachable"; exit 1; }
mongosh --quiet --eval 'db.runCommand({ping:1}).ok' chessguru >/dev/null 2>&1 || { echo "[run-m1] mongo not reachable"; exit 1; }

echo "[run-m1] starting engine e1, e2 + gateway gw1"
...
echo "[run-m1] running verifier"
WS_URL=ws://127.0.0.1:18080/ws WS_HTTP=http://127.0.0.1:18080 node "$ROOT/scripts/m1-verify.mjs"; rc=$?
