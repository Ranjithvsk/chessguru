#!/usr/bin/env bash
# Boot Redis-backed M0 cluster (2 engine nodes + 1 gateway), run the verifier,
# tear everything down. Run from v2/:  bash scripts/run-m0.sh
set -u
cd "$(dirname "$0")/.."   # → v2/
ROOT="$(pwd)"
...
redis-cli ping >/dev/null 2>&1 || { echo "[run-m0] redis not reachable"; exit 1; }

echo "[run-m0] starting engine e1, e2 + gateway gw1"
...
# wait for the gateway to accept HTTP
...
echo "[run-m0] running verifier"
WS_URL=ws://127.0.0.1:18080/ws WS_HTTP=http://127.0.0.1:18080 node "$ROOT/scripts/m0-verify.mjs"; rc=$?
