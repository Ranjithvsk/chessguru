// Shared batch admin panel — used by /fees/batches AND /academy dashboard.
//
// Extracted 2026-09-01 (was duplicated across pages/FeesBatches.tsx and
// pages/AcademyDashboard.tsx). Owner ask: "we moved batch from Academy to
// fees, show that in academy also". Same widget in both places — one source
// of truth so future edits ship to both surfaces.
//
// Data source: /api/academy/batches (unchanged). API stays in the academy
// module; this is a UI mount point.
...
// ---- BatchClassStrip: compact "next classes" list inside a batch card ----
...
// ---- BatchesPanel: main widget ------------------------------------------
export default function BatchesPanel({
  students, coaches = [], isOwner = false, classes = [], title,
}: {
  students: any[];
  coaches?: any[];
  isOwner?: boolean;
  classes?: ClassRow[];
  /** Header title override — /academy renders "👥 Batches", /fees/batches
   *  uses "Your batches" by default. */
  title?: string;
}) {
  ...
  return (
    <section className="rounded-2xl border border-ink-700 bg-ink-900 p-5">
      <div className="mb-3 flex items-center justify-between">
        <h2 className="font-display text-lg text-white">{title ?? t("Your batches")} <span className="ml-2 text-xs font-normal text-ink-400">({(batches as any[]).length})</span></h2>
        {!creating && !editingId && (
