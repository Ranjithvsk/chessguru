    ...
    <div className="mx-auto max-w-2xl space-y-4 pb-24">
      ...
      {steps.length > 0 && (
        <div className="rounded-xl2 border border-brand-700/50 bg-brand-900/25 p-5">
          ...
          {/* AI-generated comic strip (only if one exists for this opening slug). */}
          <OpeningComic slug={OPENING_PRESETS.find((p) => p.name === line.name)?.id ?? null} />
...
/** Comic-strip image for an opening (generated by scripts/openings/genOpeningComic.ts).
 *  Silently hides itself if the PNG isn't available at /openings/<slug>/comic.png.
 *  Cache-busts on regeneration by reading `generatedAt` from comic.manifest.json
 *  and appending it as ?v=… — first load fetches a tiny JSON, subsequent renders
 *  reuse the same URL so browser + service-worker cache it normally. */
function OpeningComic({ slug }: { slug: string | null }) {
  const [ok, setOk] = useState(true);
  const [ver, setVer] = useState<string>(""); // manifest generatedAt → cache-buster
  useEffect(() => {
    if (!slug) return;
    setOk(true);
    const mUrl = `${import.meta.env.BASE_URL}openings/${slug}/comic.manifest.json`;
    fetch(mUrl, { cache: "no-store" })
      .then((r) => (r.ok ? r.json() : null))
      .then((m: { generatedAt?: string } | null) => {
        if (m?.generatedAt) setVer(encodeURIComponent(m.generatedAt));
        else setOk(false);
      })
      .catch(() => setOk(false));
  }, [slug]);
  if (!slug || !ok) return null;
  const src = `${import.meta.env.BASE_URL}openings/${slug}/comic.png${ver ? `?v=${ver}` : ""}`;
  return (
    <div className="mb-4 overflow-hidden rounded-lg border border-brand-800/60">
      <img src={src} alt="Opening story comic strip"
        onError={() => setOk(false)}
        className="block w-full" />
    </div>
  );
