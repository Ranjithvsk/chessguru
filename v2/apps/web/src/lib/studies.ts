...
export const STUDIES: StudyDef[] = [
  { id: "coordinates", kind: "coordinate", icon: "a1", title: "Coordinate Training", blurb: "Place pieces by coordinate",
    detail: "A square + piece appears (e.g. \u201cBlack Rook \u2192 e7\u201d) and you tap that square to place it. Coordinates are hidden — the classic way to learn the board. 45-second sprint.", mateIn: "45s sprint", range: [400, 1200] },
  { id: "memory-palace", kind: "memory", icon: "\ud83c\udff0", title: "Memory Palace", blurb: "A funny picture on every square",
    detail: "The memory-champion trick: each of the 64 squares gets a silly, unforgettable scene (a1 = Ant on the Sun, h8 = Hedgehog that Ate too much). Pick from 11 picture sets \u2014 Easy, Animals, Mythology, Space, Ocean, Jungle, Fairy Tales, Superheroes, Cartoons, Vehicles, Food. Explore the board, then quiz yourself both ways.", mateIn: "11 sets \u00b7 no timer", range: [800, 1600] },
  { id: "opening-memory", kind: "memory", icon: "\u265f\ufe0f", title: "Opening Memory", blurb: "Memorize an opening as a story",
    detail: "Learn an opening the memory-champion way: every move becomes a hero visiting a square\u2019s funny picture (its anchor). Step through the Italian, Ruy Lopez, Scotch or Queen\u2019s Gambit \u2014 or send a line over from the Opening explorer \u2014 in any of the 11 picture sets.", mateIn: "anchors \u00b7 step-through", range: [900, 1700] },
  { id: "daily", kind: "memory", icon: "\ud83d\udd01", title: "Daily Study", blurb: "Your spaced-repetition review queue",
    detail: "The FSRS-scheduled review queue. Every opening you activate for study generates cards \u2014 one per move in the mainline, plus plan + structure cards. Grade Again/Hard/Good/Easy after each; the scheduler picks when to bring the card back so it lands right before you'd forget. Repertoire openings jump the queue.", mateIn: "FSRS \u00b7 daily cadence" },
  ...
  { id: "prep-test", kind: "memory", icon: "\u23f1\ufe0f", title: "Prep-test", blurb: "5-minute rapid drill before a game",
    detail: "You're about to play \u2014 pick White, Black-vs-1.e4, or Black-vs-1.d4, and we serve the 15 most-critical mainline positions from your activated openings on that side. Rapid-fire: reveal, grade, next. Grades still feed FSRS so a pre-match blunder gets rescheduled for tomorrow.", mateIn: "15 cards \u00b7 pick your side", range: [1000, 1800] },
  ...
  { id: "progress", kind: "memory", icon: "\ud83d\udcca", title: "Progress", blurb: "Streak, retention, mastery by family",
    detail: "Your Memory Master 500 report card: cards mastered vs learning, retention rate, streak days, reviews-per-day for the past 30 days, upcoming due-load for the next 30 days, and a mastery bar per opening family. All computed locally from your FSRS state \u2014 no accounts, no sync.", mateIn: "streak \u00b7 retention \u00b7 load" },
  ...
  { id: "repertoire", kind: "memory", icon: "\ud83c\udfaf", title: "Build my repertoire", blurb: "10 questions \u2192 your 30-40 openings",
    detail: "Answer 10 questions about your rating, time, style, and defence preferences \u2014 and the wizard picks your personalised opening repertoire from the 500. Kept in your browser only; redo whenever your style changes. When the FSRS card engine ships, your repertoire's openings get prioritised in the daily study queue.", mateIn: "10 questions \u00b7 personalised", range: [900, 1600] },
  { id: "queen-mate", kind: "mate", pieces: ["Q"], icon: "♛", title: "Queen Mate", blurb: "King + Queen vs King",
    detail: "The first checkmate to master. Box the lone king to an edge with the queen, bring your king up, and mate — without stalemating.", mateIn: "Mate in ≤ 10", range: [400, 900] },
  { id: "rook-mate", kind: "mate", pieces: ["R"], icon: "♜", title: "Rook Mate", blurb: "King + Rook vs King",
    detail: "The classic box / ladder mate. Cut the king off with the rook, oppose with your king, and drive it to the edge.", mateIn: "Mate in ≤ 16", range: [500, 1000] },
  { id: "two-rook-mate", kind: "mate", pieces: ["R", "R"], icon: "♜♜", title: "Double Rook Mate", blurb: "King + 2 Rooks vs King",
    detail: "The easiest mate of all — the two-rook 'lawnmower'. Roll the rooks rank by rank to push the king off the board. Your king isn't even needed.", mateIn: "Mate in ≤ 7", range: [400, 800] },
  { id: "two-bishop-mate", kind: "mate", pieces: ["B", "B"], icon: "♝♝", title: "Two-Bishop Mate", blurb: "King + 2 Bishops vs King",
    detail: "Two bishops on opposite colours mate the lone king in a corner. Coordinate the bishops to build a wall and use your king to herd it.", mateIn: "Mate in ≤ 19", range: [1400, 1800] },
  { id: "bishop-knight-mate", kind: "mate", pieces: ["B", "N"], icon: "♝♞", title: "Bishop + Knight Mate", blurb: "King + Bishop + Knight vs King",
    detail: "The hardest basic mate. You must drive the king to a corner of the BISHOP's colour. Needs precise king+piece coordination (the 'W' manoeuvre).", mateIn: "Mate in ≤ 33 · hardest", range: [1700, 2100] },
  { id: "pawn-endgames", kind: "pawnEnd", icon: "♟", title: "Pawn Endgames · Rated", blurb: "Real pawn endings at your level",
    detail: "The rated follow-up to the Promote One Pawn course: real pawn endgames from Dvoretsky's Endgame Manual and the Lichess puzzle base, matched to your rating. You play White against full-strength Stockfish — promote a pawn and checkmate, or hold the theoretical draw. Ratings run 600 to 2800+, so even experienced players will find trouble.", mateIn: "Rated · win or hold", range: [600, 2800] },
  { id: "stop-the-pawn", kind: "stopPawn", pieces: ["Q"], icon: "♛♟", title: "Queen vs Pawns", blurb: "King + Queen vs King + Pawns (pick 1–4)",
    detail: "Your opponent has passed pawns racing to promote. Pick how many pawns (1, 2, 3 or 4), capture or blockade them with the queen, then checkmate. If a pawn promotes you can still try to win the new queen — only a real draw or getting mated ends it.", mateIn: "1–4 pawns", range: [700, 1200] },
  { id: "rook-stop-pawn", kind: "stopPawn", pieces: ["R"], icon: "♜♟", title: "Rook vs Pawns", blurb: "King + Rook vs King + Pawns (pick 1–4)",
    detail: "Trickier than with a queen — your rook must catch the runners. Pick 1–4 pawns, get behind or in front of each, win them, then mate. If a pawn promotes you can still try to win it.", mateIn: "1–4 pawns", range: [900, 1400] },
  { id: "zugzwang", kind: "pawnEnd", icon: "⚔️", title: "Zugzwang", blurb: "The move you don’t want to make",
    detail: "11 canonical zugzwang positions across 6 pattern classes — Hooper KP-vs-K, the classical trébuchet, K+P opposition with wrong-rook-pawn draw, Lucena, Sämisch–Nimzowitsch 1923 (Immortal Zugzwang), Fischer–Rossetto 1959, Réti 1921, Saavedra 1895. Study mode reveals mechanism + source; Practice mode hides the answer and asks you to play the correct move on the board.", mateIn: "6 classes · study + practice", range: [1200, 2000] },
];
