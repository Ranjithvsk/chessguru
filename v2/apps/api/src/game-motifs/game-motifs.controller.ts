...
export class GameMotifsController {
  ...
  // GET /api/game-motifs/open/:gameId/:ply — puts the moment into the viewer's "🎯 Game awards" study and
  // redirects to the My Studies board (a plain link, so it works from the leaderboard and from emails)
  @Get("open/:gameId/:ply") async open(@Req() req: any, @Res() res: any, @Param("gameId") gameId: string, @Param("ply") ply: string) {
    const r = await this.svc.openMoment(req.session, gameId, Number(ply));
