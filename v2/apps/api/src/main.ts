...
async function bootstrap() {
  ...
  // Support attachments forwarded to pos-api. Raw body so the bytes pass
  // through untouched — the widget sends one file per request.
  app.use("/api/support/attachment", expressLib.raw({ type: ["image/*", "video/*", "audio/*", "application/*", "text/*"], limit: "11mb" }));
