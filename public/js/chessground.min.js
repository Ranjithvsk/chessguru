var __cg = (() => {
  ...
  var isFireMac = memo(
    () => !("ontouchstart" in window) && ["macintosh", "firefox"].every((x) => navigator.userAgent.toLowerCase().includes(x))
  );
  ...
  function premove(state, key) {
    ...
    const color = piece.color, friendlies = new Map([...pieces].filter(([_, p]) => p.color === color)), enemies = new Map([...pieces].filter(([_, p]) => p.color === opposite(color))), orig = { key, pos: key2pos(key) }, mobility = (ctx) => mobilityByRole[piece.role](ctx) && state.premovable.additionalPremoveRequirements(ctx), partialCtx = {
      ...
      enemies,
      color,
      rookFilesFriendlies: Array.from(pieces).filter(
  ...
  function tryAutoCastle(state, orig, dest) {
    ...
    if (!rook2 || rook2.color !== king2.color || rook2.role !== "rook") return false;
  ...
  function setSelected(state, key) {
    state.selected = key;
    if (!isPremovable(state, key)) state.premovable.dests = void 0;
  ...
  function getSnappedKeyAtDomPos(orig, pos, asWhite, bounds) {
    ...
    const validSnapPos = allPos.filter(
      (pos2) => samePos(origPos, pos2) || queenDir(origPos[0], origPos[1], pos2[0], pos2[1]) || knightDir(origPos[0], origPos[1], pos2[0], pos2[1])
    );
    const validSnapCenters = validSnapPos.map(
      (pos2) => computeSquareCenter(pos2keyUnsafe(pos2), asWhite, bounds)
    );
  ...
  function configure(state, config) {
    if (config.movable?.dests) state.movable.dests = void 0;
    ...
    if ("lastMove" in config && !config.lastMove) state.lastMove = void 0;
  ...
  function processDrag(s) {
    requestAnimationFrame(() => {
      ...
      if (!cur) return;
      if (s.animation.current?.plan.anims.has(cur.orig)) s.animation.current = void 0;
      ...
      if (!origPiece || !samePiece(origPiece, cur.piece)) cancel2(s);
        ...
        if (cur.started) {
          ...
          else cur.keyHasChanged ||= cur.orig !== getKeyAtDomPos(cur.pos, whitePov(s), bounds);
        }
      }
      processDrag(s);
    });
  ...
  function start3(state, redrawAll) {
    ...
    return {
      set(config) {
        if (config.orientation && config.orientation !== state.orientation) toggleOrientation2();
        applyAnimation(state, config);
      ...
      selectSquare(key, force) {
        if (key) anim((state2) => selectSquare(state2, key, force), state);
  ...
  // src/autoPieces.ts
  function render2(state, autoPieceEl) {
  ...
  function renderShape(state, { shape, hash: hash2 }, bounds) {
  ...
  function computeSquareClasses(s) {
    ...
    if (premove2) for (const k of premove2) addSquare(squares, k, "current-premove");
  ...
  function renderSvg(state, els) {
    ...
    syncShapes2(shapes, els, (s) => renderShape2(state, s, d.brushes, dests, bounds));
  }
  function syncDefs(d, shapes, els) {
    for (const shapesEl of [els.shapes, els.shapesBelow]) {
      ...
      for (const s of thisPlane.filter((s2) => s2.shape.dest && s2.shape.brush)) {
        ...
        if (key && color) brushes2.set(key, { key, color, opacity: 1, lineWidth: 1 });
  ...
  function syncShapes2(shapes, els, renderShape3) {
  ...
  var pieceHash = (piece) => [piece.color, piece.role, piece.scale].filter(Boolean).join(",");
  var modifiersHash = (m) => [m.lineWidth, m.hilite].filter(Boolean).join(",");
  function textHash(s) {
    let h = 0;
  ...
  function renderShape2(state, { shape, current, pendingErase, hash: hash2 }, brushes2, dests, bounds) {
    ...
    if (brush) {
      ...
      else el.appendChild(renderCircle(brushes2[shape.brush], from, current, bounds, pendingErase));
    }
    if (shape.label) {
  ...
  function Chessground(element, config) {
    ...
    function redrawAll() {
      ...
      const elements = renderWrap(element, maybeState), bounds = memo(() => elements.board.getBoundingClientRect()), redrawNow = (skipSvg) => {
        render3(state);
        if (elements.autoPieces) render2(state, elements.autoPieces);
        if (!skipSvg && elements.shapes) renderSvg(state, elements);
      }, onResize = () => {
        updateBounds(state);
        renderResized2(state);
        if (elements.autoPieces) renderResized(state);
      };
...
window.Chessground = {Chessground: __cg.Chessground};
