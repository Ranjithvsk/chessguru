# ChessGuru — PROJECT_MASTER Index

_Organized docs hub, modelled on the Dream World Plants PROJECT_MASTER pattern._
_Created 2026-05-29 · expanded the same day with a full codebase read. Live: https://harinitharanjith.com_

> **Authority rule:** for *current* behaviour, **trust the code**. These docs explain *why* and
> capture state the code can't. Where a doc and the code disagree, the code wins — then fix the doc.
>
> **Working rule:** every change → a session note + a commit; every idea → a file in this folder.
> See [knowledge/11-working-rules.md](knowledge/11-working-rules.md).

## Start here
- **[knowledge/00-architecture-overview.md](knowledge/00-architecture-overview.md)** — 1-page system map + core flows

## Knowledge
- [01-infra.md](knowledge/01-infra.md) — host, ports, domain, DNS/SSL, stack, PM2, services
- [02-app-and-db.md](knowledge/02-app-and-db.md) — app summary, subsystems, file map, DB, API surface
- [03-rules-and-gotchas.md](knowledge/03-rules-and-gotchas.md) — conventions, test-page rule, lessons table
- [04-backend-deep-dive.md](knowledge/04-backend-deep-dive.md) — server3, routes.js selection algorithm, glicko2, schemas, auth
- [05-engine-pipeline.md](knowledge/05-engine-pipeline.md) — runner, updater, Stockfish extractor, cook.js, engines.json
- [06-frontend.md](knowledge/06-frontend.md) — every page (gameplay + admin/tools) and its client flow
- [07-pools-and-maintenance-scripts.md](knowledge/07-pools-and-maintenance-scripts.md) — the pools concept + every script
- [08-api-reference.md](knowledge/08-api-reference.md) — complete endpoint reference
- [09-links-libraries-resources.md](knowledge/09-links-libraries-resources.md) — URLs, libraries, external services, upstream ports
- ⚠️ [10-known-issues-and-risks.md](knowledge/10-known-issues-and-risks.md) — consolidated bug/security register (read this)
- ⭐ [11-working-rules.md](knowledge/11-working-rules.md) — **commit & documentation discipline (change→note→commit; ideas→folder)**
- 🚀 [17-v2-deploy-playbook.md](knowledge/17-v2-deploy-playbook.md) — **step-by-step deploy for v2 (NestJS + Vite): merge → build → rsync → restart → smoke → session note**
- ⭐ [18-board-scan-what-actually-fails.md](knowledge/18-board-scan-what-actually-fails.md) — **MEASURED: framing decides scan success, not the classifier**; 3 failure modes; the guard that looks useful and isn't; 3 auto-crop approaches that failed
- ⭐ [19-vision-training-assets.md](knowledge/19-vision-training-assets.md) — every diagram/photo pile, which have ground truth, and the 405k-image pile no trainer was reading
- ⭐ [20-dream-ocr.md](knowledge/20-dream-ocr.md) — **MEASURED: chess legality beats a fourth OCR engine**; 89%→99% on book noise; the 4 bugs only measuring found; why false certainty is the number to watch
- ⭐ [21-book-ingest-pitfalls.md](knowledge/21-book-ingest-pitfalls.md) — **how book diagrams get LOST or misread**: the detector's thumbnail is clipped (crop from `box`), an illegal FEN silently DELETES the diagram, `warped=` skips the margin trim, and `conf` is a mean that hides all of it — use `minConf`
- ⭐ [22-scan-speed-vs-accuracy.md](knowledge/22-scan-speed-vs-accuracy.md) — **measure before shipping a speed change**: two attempts cost accuracy; the extractor detects at 640 but CROPS FROM FULL RES, and the correction loop was dead for everyone

## Decisions (ADRs)
- [ADR-0001](decisions/ADR-0001-server3-is-the-entrypoint.md) — `server3.js` is the only live entrypoint; routes load inside the connect callback
- [ADR-0002](decisions/ADR-0002-auth-bypasses-mongoose.md) — auth uses the raw MongoDB driver (BSON-Binary `bpass`)
- [ADR-0003](decisions/ADR-0003-glicko2-lichess-exact.md) — Glicko-2 is a frozen Lichess port
- [ADR-0004](decisions/ADR-0004-express-v5-named-wildcard.md) — Express v5 catch-all uses `/*splat`
- [ADR-0005](decisions/ADR-0005-test-page-workflow.md) — new features ship via `public/test/` first
- [ADR-0006](decisions/ADR-0006-engine-pipeline-self-feeds-puzzles.md) — puzzles are self-generated by the engine pipeline
- [ADR-0007](decisions/ADR-0007-precomputed-pools.md) — selection uses pre-computed pools, not live sampling
- [ADR-0008](decisions/ADR-0008-realtime-stack.md) — realtime online-play stack (uWS gateway, roll-our-own grain directory, JSON-versioned protocol); **Proposed**, pending owner + infra sign-off

## Database
- [database/schema.md](database/schema.md) — schemas, all collections + counts, the puzzle field-vocabulary divergence

## Plans
- [Academy online tournaments (Play → Tournaments, Swiss via JaVaFo)](plans/academy-online-tournaments.md) — PROPOSED 2026-09-25
- ⭐ [plans/online-play-international.md](plans/online-play-international.md) — PROPOSED: Lichess-level platform at international scale (separate WS + game-engine tiers, Redis bus, fishnet, anti-cheat, tournaments); infra decision flagged
  - ↳ ⭐ [plans/online-play-realtime-architecture.md](plans/online-play-realtime-architecture.md) — PROPOSED (deep): realtime play architecture — grain directory (Orleans-style single-writer), uWS gateway tier, wire protocol, server-truth clocks, failure/recovery, M0–M5 build phases — **M0–M5 BUILT & verified — Phase 1 complete**
    - ↳ [plans/online-play-m0-walking-skeleton.md](plans/online-play-m0-walking-skeleton.md) — ✅ BUILT & VERIFIED (13/13): M0 walking skeleton — concrete monorepo packages (apps/ws, apps/game-engine, packages/protocol) + 6 exit criteria proving directory/single-writer/rehydration
- ⭐ [plans/human-like-bot-opponent.md](plans/human-like-bot-opponent.md) — PROPOSED: Maia-3 opponent when the seek pool is empty — plays at the seeker's rating, Indian name, fully disguised, rated; **AGPL: never fork/patch the engine**; rated go-live still gated — calibration round 1 measured **0.46x Elo compression**, so `SelfElo` must not map 1:1 to student rating
- ⭐ [plans/book-reader-upload-your-own.md](plans/book-reader-upload-your-own.md) — PROPOSED: "My Books" — upload a PDF/EPUB, the vision pipeline reads every diagram once at ingest, reader plays them; finishes the "clickable diagrams coming via vision extractor" note already sitting on Yusupov
- ⭐ [plans/board-scan-roadmap.md](plans/board-scan-roadmap.md) — board scan ordered by leverage: what shipped 2026-09-09, the extractor retrain, orientation-from-coordinates, the big-pile repoint, classifier pseudo-labelling, ChessVision oracle (blocked on API key); plus what we deliberately won't do
- [plans/own-opening-explorer.md](plans/own-opening-explorer.md) — IN PROGRESS: own NestJS+Mongo opening book (position-keyed), Lichess-compatible /api/explorer
- [plans/ovh-migration.md](plans/ovh-migration.md) — DONE (co-located on the DreamWorld OVH VPS)
- [plans/code-cleanup.md](plans/code-cleanup.md) — archived dead files + remaining follow-ups
- [plans/feedback-ui-lichess-exact.md](plans/feedback-ui-lichess-exact.md) — pending feature

## Research
- [research/lichess-source-reads.md](research/lichess-source-reads.md) — which Lichess `lila` / `lichess-puzzler` sources were ported

## Sessions
- ⚠️ [2026-09-10-extractor-retrain-rejected.md](sessions/2026-09-10-extractor-retrain-rejected.md) — 40k composites and 6h of training scored **0.954 mask mAP** on synthetic data and changed NOTHING end-to-end; rejected, not installed; the detector metric was marking its own homework
- ⭐ [2026-09-09-dream-ocr.md](sessions/2026-09-09-dream-ocr.md) — four OCR engines combined as asked, but **chess legality is what got it to 99%**, not the engines; the first version passed a hand-made test 20/20 and scored 55%→58% on a real harness, which then exposed six bugs
- ⭐ [2026-09-07-play-bot-ladder-shipped.md](sessions/2026-09-07-play-bot-ladder-shipped.md) — `play-bot` now answers a lone seeker from an **lc0 + Maia-1 1100–1900 process pool** (25% variety: Maia-3 in-band, weak UCI engines + Elo-capped Stockfish at 1700+); shipped to the ubuntu clone by rsync (play services are NOT symlinked) and verified with a live seek; `bot_games` records `engine`; gateway `hello` trusts any token as userId — fix before rated
- ⭐ [2026-09-07-lc0-maia1-install.md](sessions/2026-09-07-lc0-maia1-install.md) — lc0 v0.32.1 built from source (no Linux release binaries exist) + the nine Maia-1 nets 1100–1900; **policy-only, so every `go` must be `go nodes 1`**; maia-1100 falls for the Blackburne trap and 1200+ don't, proving real strength separation without Maia-3's 0.46x calibration gate
- [2026-09-06-low-rated-engine-survey.md](sessions/2026-09-06-low-rated-engine-survey.md) — built 10 open-source engines looking for low-rated bot opponents: **6 work but all cluster at ~1800–2000 with no strength dial; every genuinely sub-1500 engine is abandoned and broken** → stay on Maia; lc0+Maia-1 weights or Stockfish `UCI_Elo` are the only real dials
- ⚠️ [2026-09-06-play-board-dead-and-clock-ticks.md](sessions/2026-09-06-play-board-dead-and-clock-ticks.md) — no move was possible in Play: **chessground binds input listeners only while `viewOnly` is false, and only at construct/`redrawAll` time**, so a board mounted as a spectator (or flipped to black) is dead forever; also a stale cached board rect that put clicks a rank off, plus locally-interpolated clocks
- ⚠️ [2026-09-06-maia-bot-opponent-live.md](sessions/2026-09-06-maia-bot-opponent-live.md) — quick pairing is now answered by a Maia-3 bot under a human Indian name (`apps/bot-player`, **unrated only** — the strength mapping has no human anchor yet); also fixed ghost seeks surviving a closed tab, and colour following arrival order
- ⚠️ [2026-09-06-online-play-never-deployed.md](sessions/2026-09-06-online-play-never-deployed.md) — Play sat on "Waiting for a match" because online play **had no backend in production**: the bundle dialled `ws://localhost:18080` and ws/lobby/game-engine were never in pm2 → `ecosystem.play.config.cjs`, nginx `location = /ws`, same-origin WS URL
- [2026-09-04-mail-health-monitoring.md](sessions/2026-09-04-mail-health-monitoring.md) — mail can't alert about its own outage: 2-min `/health` probe + `sendMail` observer → header banner for admins, `errorEvents` row, and an emailed postmortem the moment it recovers
- ⚠️ [2026-09-04-error-alerting.md](sessions/2026-09-04-error-alerting.md) — auto-alerting on server 5xx / browser crashes / slow requests → `errorEvents` + throttled email + `/admin/errors`; **found all ChessGuru email dead since ~30 Aug (dwotp tunnel publickey denied)**
- [2026-09-04-puzzle-picker-slow-and-repeating.md](sessions/2026-09-04-puzzle-picker-slow-and-repeating.md) — 50-id `paths` pools ran dry for active students → 26s `$sample` fallback + endless repeats; neighbour-band failover, never-seen-first, indexed seek instead of `$sample`, pools 50→1000
- [2026-05-30-own-opening-explorer.md](sessions/2026-05-30-own-opening-explorer.md) — building our OWN opening explorer (Lichess 401s our datacenter IP); ingestion foundation done (ECO names + position tree)
- [2026-05-30-v2-remove-themes-section.md](sessions/2026-05-30-v2-remove-themes-section.md) — dropped the redundant Themes page (same solver as Puzzles, which already has a theme dropdown); /theme now redirects to /
- [2026-05-29-v2-feedback-ui-test-harness.md](sessions/2026-05-29-v2-feedback-ui-test-harness.md) — Lichess-exact feedback block built as a shared component + isolated test page (`/test/feedback-ui`); typechecks; production merge gated on review (ADR-0005)
- [2026-05-29-v2-puzzle-replay-dedup.md](sessions/2026-05-29-v2-puzzle-replay-dedup.md) — v2 selector now excludes already-played puzzles per user (rounds $nin) + guest/fallback safe
- [2026-05-29-homepage-theme-and-mobile-overflow.md](sessions/2026-05-29-homepage-theme-and-mobile-overflow.md) — theme picker surfaced + mobile overflow fixed (home/blindfold/theme)
- [2026-05-29-full-codebase-documentation.md](sessions/2026-05-29-full-codebase-documentation.md) — full read + docs
- [2026-04-25-sessions-1-4-auth-rating-login.md](sessions/2026-04-25-sessions-1-4-auth-rating-login.md) — auth/rating/login (from the old monolith)

---
_The original flat `PROJECT_MASTER.md` and `CLAUDE.md` remain in the repo root. `CLAUDE.md` is the
authoritative engineering guide for Claude Code; this folder is the human-readable knowledge hub._

