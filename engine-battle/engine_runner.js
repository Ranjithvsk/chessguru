...
let Game,Tournament;
function loadEngines(){const reg=JSON.parse(fs.readFileSync(ENGINES_JSON));const out=[];const seen=new Set();for(const e of reg.engines){if(!e.binary)continue;const bp=path.join(ENGINES_DIR,e.binary);if(fs.existsSync(bp)&&!seen.has(bp)){seen.add(bp);out.push({...e,path:bp});}}return out;}
class UCIEngine extends EventEmitter{
  ...
  think(fen,moves,ms){return new Promise((res,rej)=>{this.send('ucinewgame');this.send('position fen '+fen+(moves.length?' moves '+moves.join(' '):''));const t=setTimeout(()=>rej(new Error('timeout')),ms+15000);this.once('bestmove',mv=>{clearTimeout(t);res(mv);});this.send('go movetime '+ms);});}
  quit(){try{this.send('quit');}catch(e){}setTimeout(()=>{try{this.proc&&this.proc.kill();}catch(e){}},500);}
}
let Chess=null;try{Chess=require('/home/ubuntu/chessguru/node_modules/chess.js').Chess;}catch(e){}
let wss,clients=new Set();
function bc(msg){const d=JSON.stringify(msg);for(const c of clients)if(c.readyState===1)c.send(d);}
async function runGame(W,B,ms,tid,round){
  const FEN='rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1';
  const moves=[];let chess=Chess?new Chess():null,result='*',term='';
  const gd={tournamentId:tid,round,whiteId:W.engine.id,whiteName:W.engine.name,blackId:B.engine.id,blackName:B.engine.name,result:'*',moves:[],thinkMs:ms,startedAt:new Date(),whiteElo:W.engine.elo,blackElo:B.engine.elo};
  bc({type:'game_start',game:gd,round});
  try{
    let cur=W,oth=B,side='white';
    for(let p=0;p<300;p++){
      const mv=await cur.think(FEN,moves,ms);
...
async function tourney(cfg){
  const{engineIds,thinkMs,maxGames,name}=cfg;
  ...
  bc({type:'tournament_start',tournament:t,engines:sel.map(e=>({id:e.id,name:e.name,elo:e.elo})),totalGames:limit});
  const done=[];
  for(let i=0;i<limit&&running;i++){
    ...
    try{await Promise.all([we.start(),be.start()]);const gm=await runGame(we,be,thinkMs,tid,i+1);done.push(gm);
